...
final class ClientTable extends PowerGridComponent
{
    ...
    public function setUp(): array
    {
        ...
        return [
            ...
            Header::make()->showSearchInput()->showToggleColumns()->includeViewOnTop('components.header-top'),
            Footer::make()
                ->showPerPage()
    ...
    public function destroy($rowId): void
    {
        try {
            ...
            session()->flash('alert-class', 'bg-green-50 text-green-800 border border-green-800');
        } catch (\Exception $e) {
            session()->flash('message', 'Failed to delete user: ' . $e->getMessage());
            session()->flash('alert-class', 'bg-red-50 text-red-800 border border-red-800');
        }
    }
